...
const increaseHour = () => {
    ...
    if (hour >= 24)
        ...
        hourCntr.innerText = `${hour < 10 ? "0" + hour : hour}`;
};
const increaseMin = () => {
    ...
    if (min >= 60)
        ...
        minCntr.innerText = `${min < 10 ? "0" + min : min}`;
};
const increaseSec = () => {
    ...
    if (sec >= 60)
        ...
        secCntr.innerText = `${sec < 10 ? "0" + sec : sec}`;
};
...
const takeLap = () => {
    if (lapBtn.classList.contains("disable"))
        return;
...
(_a = document.getElementById("clearLap")) === null || _a === void 0 ? void 0 : _a.addEventListener("click", () => {
